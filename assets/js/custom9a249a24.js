...
$(document).ready(async function () {
  if (typeof web3 !== 'undefined') {
    ...
    if (bsc && bsc.on) {
      window.chainid = await web3.bnb.getChainId();

      bsc.on('chainChanged', async function (networkId) {
        await setupConnected();
        localStorage.clear();
        
        if (networkId !== Bsc_MAINNET_ID) {
          toastr.warning('Please switch to Bsc Mainnet');
        }

        var connect = await bsc.request({ method: 'bnb_accounts' });
        if (connect.length == 0) {
          bsc.enable().then(
            value => {
	...
	async function setupConnected(first = null) {
		...
		if (currentNetwork === targetNetwork || (currentNetwork === 'unknown' && chainId === targetChainId)) {
		  ...
		  window.contract_token = await new web3.bnb.Contract(token_abi, token_contract);
		  window.bnb_contract = await new web3.bnb.Contract(bnb_presale_abi, bnb_presale);
	  
		  window.decimals = await contract_token.methods.decimals().call();
		  window.accounts = await bsc.request({ method: 'bsc_requestAccounts' });
		  window.account = accounts[0];
		  await tokenRate();
	  
		  var balance = await web3.bsc.getBalance(bnb_presale);
		  ...
		  if (localStorage.getItem(ADDRESS) != null && localStorage.getItem(ADDRESS).length > 0) {
			web3.bnb.defaultAccount = localStorage.getItem(ADDRESS);
		  ...
		  closeApp();
		  if (first !== true) {
			toastr.error('Connect to the Binance smart chain network');
		  } else {
			
			try {
			  await bsc.request({
				method: 'wallet_addbscChain',
				params: [
				  {
			  ...
			  toastr.success('Binance smart chain network added to MetaMask');
			  
			  location.reload();
			} catch (error) {
			  console.error(error);
			  toastr.error('Failed to add Binance smart chain network to MetaMask');
			}
		  }
		}
	  }
	  ...
	  function getNetworkName(chainId) {
		...
		bsc.on('accountsChanged', async function (accounts) {
	    	localStorage.clear()
	...
	$('input[id=bnb_value]').on('input',async function(){
			var network_id = await web3.bnb.net.getId();
										...
										await bnb_contract.methods.buyTokens(account).send({ 'value' : value, 'from' : account},function(err,result){
											if(result){
												var interval = setInterval(async function(){ 
					       							var receipt =  await web3.bnb.getTransactionReceipt(result);
